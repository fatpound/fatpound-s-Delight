...
matrix matrix::returnMultipledMatrix(matrix* second)
{
	if (_col != second->_row)
	{
		return NULL;
...
matrix matrix::returnMinor(matrix* source, int row, int col)
{
...
matrix matrix::returnAdj(matrix* source)
{
	...
	for (int i = 0; i < source->_row; i++)
	{
		for (int j = 0; j < source->_col; j++)
		{
			matrix temp = source->returnMinor(source, i, j);
...
int matrix::calculateDeterminant(matrix* source)
{
	...
	for (int i = 0; i < source->_row; i++)
	{
		matrix newm = returnMinor(source, i, 0);
